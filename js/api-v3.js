...
function salvarAtendimento(atendimento) {
	...
	const atendimentos =
		JSON.parse(localStorage.getItem(chaveAtendimento)) || [];
...
function atualizarAtendimento(atendimentoAtualizado) {
	...
	atendimentos = atendimentos.map((atendimento) =>
		atendimento.id === atendimentoAtualizado.id
...
function editarCachorro(idCachorro) {
	// Monta a URL com o parâmetro idCachorro
	const url = `./novo-pet.html?idCachorro=${idCachorro}`;
